...
class HelperAuthorization: NSObject {
    ...
    func commandInfo() -> Dictionary<String, Any> {
        ...
        [
            NSStringFromSelector(#selector(HelperProtocol.startConfirmedProxy(reply:))) :
                [AuthorizationRightKey.rightName : "com.insanj.whilom.administratorPrivileges", AuthorizationRightKey.rightDefaultRule : ruleAdminRightsExtended, AuthorizationRightKey.rightDescription : "Prevent your computer from going to sleep"]
        ]
        return sCommandInfo
    }
     ...
     */
    func checkAuthorization(authData: NSData?, command: String) -> AuthorizationRef? {
